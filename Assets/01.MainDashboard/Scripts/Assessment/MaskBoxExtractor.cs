...
{
    /// <summary>
    /// DeepLabV3+의 픽셀 분할 결과에서 결함 영역을 감싸는 사각형을 뽑아낸다.
    ///
    /// RT-DETR이 아니라 마스크에서 사각형을 만드는 이유:
    /// 실측 검증에서 RT-DETR은 대부분의 이미지에서 임계값을 넘는 검출을 내지 못했고,
    /// 실제로 결함을 잡아낸 것은 분할 모델이었다. RT-DETR 결과만 표시하면 결함이
    /// 검출된 사진에서도 사각형이 거의 나오지 않는다.
    ///
            ...
            int maxBoxes = DefaultMaxBoxes)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                for (int x = 0; x < width; x++)
                {
                    ...
                    // 분할 모델은 배경이 0번을 차지하므로 RT-DETR 기준 id로 되돌리려면 1을 뺀다.
