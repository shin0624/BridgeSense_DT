...
// RT-DETR 검출 결과 + DeepLabV3+ 분할 결과를 하나로 묶은 이미지 분석 결과.
...
public class BridgeAnalysisResult
{
    ...
    public DeeplabResult Segmentation;       // DeepLabV3+ 픽셀 분할 맵(512x512 모델 좌표)
}

// UI 쪽에서 AI 추론이 필요할 때 유일하게 참조해야 하는 진입점.
// RtdetrModel/DeeplabModel을 직접 소유·조율하는 오케스트레이터.
//
// 다른 매니저들과 달리 이 매니저만 DontDestroyOnLoad로 씬을 넘어 유지한다.
// 두 모델 파일을 대시보드에 들어가는 순간 부담하면 화면이 그대로 멈춘다.
// StartScene에서 미리 불러두고 그대로 들고 넘어간다.
...
public class AiInferenceManager : MonoBehaviour
{
    ...
    [SerializeField] private ModelAsset deeplabModelAsset;  // Assets/06.AI/models/deeplabv3plus.onnx를 인스펙터에서 연결

    // 원안 기본값은 0.5였지만, TestImages 실측 검증에서 실제 score가
    ...
    private DeeplabModel deeplabModel; // DeepLabV3+ 래퍼 인스턴스

    /// <summary>두 모델이 모두 준비됐는지.</summary>
    public bool IsReady => rtdetrModel != null && deeplabModel != null;
    ...
    /// 이 경로로 들어오면 첫 추론 직전에 두 모델을 한 번에 읽느라 화면이 멈춘다.
    /// </summary>
    public void EnsureInitialized()
    {
        ...
        deeplabModel = new DeeplabModel(deeplabModelAsset);
    }
    ...
    public BridgeAnalysisResult AnalyzeImage(Texture2D image)
    {
        ...
        var segmentation = deeplabModel.Run(image);                    // DeepLabV3+ 분할 실행
    ...
    private void OnDestroy()
    {
        ...
        deeplabModel?.Dispose(); // DeepLabV3+ 워커·텐서 메모리 해제
