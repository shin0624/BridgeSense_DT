...
// RtdetrModel을 직접 소유·조율하는 오케스트레이터.
//
// 다른 매니저들과 달리 이 매니저만 DontDestroyOnLoad로 씬을 넘어 유지한다.
// 모델 파일을 대시보드에 들어가는 순간 부담하면 화면이 그대로 멈춘다.
...
public class AiInferenceManager : MonoBehaviour
{
    ...
    /// <summary>모델이 준비됐는지.</summary>
    public bool IsReady => rtdetrModel != null;
    ...
    /// 이 경로로 들어오면 첫 추론 직전에 모델을 한 번에 읽느라 화면이 멈춘다.
    /// </summary>
    ...
    /// <summary>
    /// 모델을 불러오며 진행 상황을 알린다. StartScene의 로딩 화면이 사용한다.
    ///
    /// 모델을 읽는 동안은 어차피 프레임이 멈추지만,
    ...
    // UI 쪽에서 호출할 유일한 진입점 - 모델의 존재를 몰라도 이미지 한 장만 넘기면 됨
    public BridgeAnalysisResult AnalyzeImage(Texture2D image)
    {
        ...
        return new BridgeAnalysisResult
        {
