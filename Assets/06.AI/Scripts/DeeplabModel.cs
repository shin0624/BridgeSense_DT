...
// DeepLabV3+ 픽셀 분할 결과 - 512x512 모델 좌표계 기준 그대로 반환한다.
// 원본 이미지 해상도로 되돌리는 업스케일은 이 클래스의 책임이 아니라 시각화 단계에서 처리한다.
public class DeeplabResult
{
    public int Width;      // 512 고정
    public int Height;     // 512 고정
    public int[] ClassMap; // 픽셀별 클래스 id(0~9), 행 우선 인덱싱: index = y * Width + x
}

// deeplabv3plus.onnx 한 개만 감싸는 래퍼 - 전처리·추론·후처리를 이 클래스 안에서 전부 캡슐화한다.
//
// smp(segmentation_models_pytorch)의 DeepLabV3Plus(encoder_name="resnet34",
// encoder_weights=None)를 AI-Hub 데이터로 처음부터 학습한 모델. 인코더 사전학습 가중치를
// 쓰지 않은 이유는 ai/CLAUDE.md 참고(구 SegFormer 베이스 nvidia/mit-b2의 사전학습 가중치가
// 비상업 전용 라이선스였던 것을 대체).
public class DeeplabModel : IDisposable
{
    private const int InputSize = 512; // deeplabv3plus_config.json: 고정 입력·출력 해상도
    private const int NumClasses = 10; // 배경/정상(0) + 결함 9종(1~9)

    // deeplabv3plus_config.json: ImageNet mean/std로 정규화(RT-DETR과 달리 여기는 정규화를 적용)
    private static readonly float[] Mean = { 0.485f, 0.456f, 0.406f };
    private static readonly float[] Std = { 0.229f, 0.224f, 0.225f };

    private readonly Worker worker;                // deeplabv3plus.onnx를 실행하는 Sentis 워커
    private readonly Tensor<float> rawInputTensor;  // 텍스처 리샘플링 결과(0~1, 정규화 전)를 담아 매 호출마다 재사용하는 텐서

    public DeeplabModel(ModelAsset modelAsset, BackendType backendType = BackendType.GPUCompute)
    {
    ...
    public DeeplabResult Run(Texture2D image)
    {
        ...
        for (int i = 0; i < rawPixels.Length; i++) // 전체 3*512*512 원소를 순회
        {
            ...
            normalizedPixels[i] = (rawPixels[i] - Mean[channel]) / Std[channel]; // 채널별 ImageNet mean/std 정규화
        }
        ...
        var logitsTensor = worker.PeekOutput("logits") as Tensor<float>; // [1,10,512,512] 픽셀별 클래스 로짓(입력과 동일한 512x512로 이미 업샘플된 상태로 출력됨)
        var logits = logitsTensor.DownloadToArray(); // GPU/워커 내부 데이터를 CPU 배열로 동기 readback

        var result = new DeeplabResult { Width = InputSize, Height = InputSize, ClassMap = new int[pixelsPerChannel] }; // 픽셀별 클래스 id를 담을 결과 객체
        ...
        for (int c = 1; c < NumClasses; c++) // 나머지 9개 결함 클래스를 바깥 루프로 순회
        {
            ...
            for (int p = 0; p < pixelsPerChannel; p++) // 이 클래스 블록 안에서는 픽셀을 처음부터 끝까지 순서대로만 읽음
            {
                ...
                {
                    bestLogit[p] = logit;
                    result.ClassMap[p] = c; // argmax 결과(픽셀별 클래스 인덱스) 갱신
                }
            }
        }
