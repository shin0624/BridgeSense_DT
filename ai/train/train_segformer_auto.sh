#!/bin/bash
# SegFormer 본 학습을 자동 재시도(폴백)하며 실행하는 래퍼 스크립트.
# RT-DETR(train_rtdetr_auto.sh)와 동일한 패턴 — 죽으면 최신 체크포인트를 찾아
# --resume-from-checkpoint로 이어서 재시작한다. 시간 기준으로 재시도를 포기하지 않고
# 완료될 때까지 계속한다.
#
# 실행: bash train_segformer_auto.sh
set -uo pipefail

OUTPUT_DIR=/workspace/ai/checkpoints/segformer
MAX_RETRIES=30
...
while [ "$attempt" -lt "$MAX_RETRIES" ]; do
  ...
  /workspace/.venv/bin/python train_segformer.py \
    ...
    --epochs 10 --batch-size 8 --gradient-accumulation-steps 4 --bf16 \
